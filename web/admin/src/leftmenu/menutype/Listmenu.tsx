...
> {
	...
	fetchData() {
		FetchWithAuth(
			`${process.env.REACT_APP_REMOTE_URL}/content/list/folder?parent=${this.props.config.root}`,
		).then((data) => {
			if (data.error === false) {
			...
			this.state.data && (
				<div className="menuitem">
					<div className="menuitem-head">
						<a href="#">
							<i className={this.props.config.icon} /> {
						...
						<div className="right">
							...
							>
								<i
									className={
										`foldable fas fa-chevron-right${(this.state.open ? " open" : "")}`
									}
								/>
							</a>
						</div>
									...
									return (
										<li>
											<NavLink to={`/main/${item.id}`}>
												<i
													className={
														`nodeicon far icon-${item.content_type} ${item.folder_type}`
													}
												/>
												{item.name}
											</NavLink>
